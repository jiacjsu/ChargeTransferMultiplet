...
class GraphFrame(wx.Frame):
    ...
    def init_plot(self):
        ...
        self.axes.set_title('L-edge X-ray Absorption Spectrum', size=12)
        self.axes.set_ylabel('Intensity (arb. unit)', size=10)
        ...
        #
        self.dat = np.loadtxt(self.seedname + '.XAS.dat')
        self.plot_data = self.axes.plot(
            self.dat[:,0], self.dat[:,1], 
            linewidth=1,
            color='blue',
            )[0]
        self.xmin = min(self.dat[:,0]) 
        self.xmax = max(self.dat[:,0]) 
        self.ymin = 0
        self.ymax = max(self.dat[:,1]) * 1.1
    ...
    def draw_plot(self):
        ...
        self.plot_data.set_xdata(self.dat[:,0])
        self.plot_data.set_ydata(self.dat[:,1])
